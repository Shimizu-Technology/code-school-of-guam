...
const CheckoutForm: React.FC<CheckoutFormProps> = ({ amount, paymentType, onSuccess }) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const { error: stripeError, paymentMethod }: PaymentMethodResult = await stripe.createPaymentMethod({
      type: 'card',
    ...
    const response = await fetch('/api/charge', {
      ...
      body: JSON.stringify({ paymentMethodId: paymentMethod?.id, amount, paymentType }),
    });
