# Code School of Guam - About Us

## Who We Are

Code School of Guam is Guam's first coding bootcamp, founded to provide high-quality, accessible coding education to the people of Guam and beyond. We're building Guam's tech ecosystem one coder at a time.

## Our Mission

To provide high-quality, accessible coding education to the people of Guam and beyond, ensuring graduates are prepared to enter the job market as software engineers.

## Our Vision

We envision transforming Guam into a tech hub by equipping local residents with the skills and real-world experience needed to succeed in the global software industry.

## What Makes Us Different

### Small Class Sizes
- Maximum 10 students per cohort
- Personalized attention and mentorship
- More 1-on-1 time with instructors

### Fully Remote & Accessible
- Classes conducted via Zoom
- Accessible from anywhere on Guam
- From Hagåtña to Dededo, Yigo to Merizo, and beyond
- No commute required

### Real-World Opportunities
- Optional internship at Shimizu Technology (unpaid, experience-focused)
- Paid TA positions for top performers
- Junior developer contract opportunities with Shimizu Technology

### Lifetime Access
- All recordings and resources forever
- Future curriculum updates included
- Ongoing support after graduation

### Local Focus, Global Skills
- Building Guam's tech community
- Skills applicable worldwide
- Local business projects and connections

## About the Founder

### Leon Shimizu
**Founder & Lead Instructor**

Leon Shimizu founded Code School of Guam with a mission to help others realize that a career in software engineering is attainable.

> "Seeing our students go from never having coded before to presenting full-stack applications with AI features in 22 weeks is incredibly rewarding. They're ready for real developer roles."

Leon brings professional experience in software development and a passion for teaching. He's here to support students every step of the way.

**Philosophy**: "If I can do it, so can you."

## Shimizu Technology

Code School of Guam is connected to Shimizu Technology, a local software development firm. This connection provides:
- Real internship opportunities
- Industry mentorship
- Job placement pathways
- Real-world project experience

## Cohort History

Code School of Guam has completed 2 cohorts so far, with the 3rd cohort starting March 2, 2026.

### Cohort 1 (2025)
- **100% Completion Rate** - All students finished the program
- **16 Weeks** - From beginner to developer
- **8 Real Projects Built** - Hands-on applications

### Cohort 2 (2025)
- **100% Completion Rate** - Every student graduated
- **22 Weeks** - Expanded program with AI Engineering added
- Students built full-stack applications with AI-powered features

### Overall Track Record
- **11 graduates** across 2 cohorts
- **100% completion rate** - Every student who enrolled has graduated
- Graduates have gone on to TA roles, junior developer positions, and teaching roles
- The upcoming March 2026 cohort will be Cohort 3

## Student Success Stories

### Graduate Testimonials

**Noah Peredo** - Graduate
> "Clear mind and trust the process. CSG will match the effort that you give it, so at the end of the day, how bad do you want it?"

**Jessica Fernandez** - Graduate
> "I am very very VERY glad I enrolled and finished it! Now, I constantly think about ways I could 'hack' my daily life by creating apps. Leon was extremely helpful in answering all my questions and providing valuable guidance."

**Alanna Cruz** - Software Engineer
> "The Code School of Guam provided me with a comprehensive curriculum covering Ruby, Rails, React, and now Python & AI Engineering. The expert guidance and hands-on projects prepared me perfectly for my software engineering career!"

### Graduate Spotlights

**Alanna Cruz** - TA & Junior Software Engineer
Alanna always felt like the "least tech-savvy" person in her family. She signed up the day before class started, not knowing if she could do it. Today she's a Teaching Assistant for the current cohort AND a Junior Software Engineer at Shimizu Technology. If she can do it, anyone can.

**Audreana Lett** - Coding Instructor at Father Duenas Memorial School
While taking our program, Audreana was already teaching at Father Duenas Memorial School. She was passionate about bringing coding to high schoolers — so we made it happen. She now teaches 15 students across two courses (Intro to Scripting with Ruby and Intro to HTML/CSS/JavaScript), showing the next generation that coding is a real career path.

## Father Duenas Memorial School Partnership

Our first high school partnership! Audreana Lett, a graduate from our very first cohort, is now teaching coding electives at Father Duenas Memorial School — Leon's own high school alma mater. She's bringing a simplified version of our curriculum to high schoolers, proving that coding is a real career path they can start exploring now.

- **15 Students** enrolled across two elective courses
- **Courses Offered**: Intro to Scripting with Ruby, Intro to HTML/CSS/JavaScript
- **Capstone projects completed** by students

## Building Guam's Tech Future

As Guam's tech ecosystem grows, Code School of Guam is committed to developing local talent that can contribute to the island's digital transformation. Our alumni network continues to expand, creating opportunities for referrals and collaboration with local businesses.

## Connect With Us

- **Website**: codeschoolofguam.com
- **Email**: codeschoolofguam@gmail.com
- **Phone**: +1 (671) 483-0219
- **Instagram**: @codeschoolofguam
- **Facebook**: facebook.com/codeschoolofguam

## Demo: See What You Can Build

Visit our Flappy Bird demo game at codeschoolofguam.com/flappy-bird to see the kind of interactive applications you'll learn to create. The game demonstrates:
- React Hooks and state management
- Canvas API for 2D graphics
- Game loops with requestAnimationFrame
- Physics simulation and collision detection
- Event handling for keyboard and touch
- Local storage for high scores
- Responsive design principles
- TypeScript for type safety

Ready to build your own games and apps? Join our March 2026 cohort!

